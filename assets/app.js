...
var jetselection = new GameImage("jetselection.png", 4);
...
var indoJet;
var currentJet;
if(localStorage.getItem(title+"CurrentJet") === null){
	localStorage.setItem(title+"CurrentJet", 0);
}
currentJet = parseInt(localStorage.getItem(title+"CurrentJet"));
var aircrafts;
...
function reset(){
	...
	switch (currentJet){
		case 1 :
			indoJetImage = new GameImage("indojet1.png", 3);
			indoJet.health = 10;
			indoJet.maxHealth = 10;
			indoJet.jetSpeed = 4;
			indoJet.firingSpeed = 7;
			indoJet.firingInterval = 8;
			break;
		case 2 :
			indoJetImage = new GameImage("indojet2.png", 3);
			indoJet.health = 10;
			indoJet.maxHealth = 10;
			indoJet.jetSpeed = 5;
			indoJet.firingSpeed = 5;
			indoJet.firingInterval = 6;
			break;
		case 3 :
			indoJetImage = new GameImage("indojet3.png", 3);
			indoJet.health = 10;
			indoJet.maxHealth = 10;
			indoJet.jetSpeed = 6;
			indoJet.firingSpeed = 3;
			indoJet.firingInterval = 4;
			break;
		default :
			indoJetImage = new GameImage("indojet.png", 3);
			indoJet.health = 10;
			indoJet.maxHealth = 10;
			indoJet.jetSpeed = 3;
			indoJet.firingSpeed = 5;
			indoJet.firingInterval = 10;
			break;
	}
...
function startScene(x){
	switch (x) {
		...
		case "about" :
			...
			ifImage.draw("still", gameWidth/2, gameHeight/2 - 200, 1, 0, 0, 0);
			btnMainMenu.draw("still", gameWidth/2, gameHeight/2 + 500, 1, 0, 0, 0);
			ctx.fillText("v1.0.0", gameWidth/2, gameHeight/2 + 180);
			ctx.fillText("Developed by Habibie.", gameWidth/2, gameHeight/2 + 200);
			ctx.fillText("Email: habibieamrullah@gmail.com", gameWidth/2, gameHeight/2 + 220);
			ctx.fillText("Website: zofiakreasi.com", gameWidth/2, gameHeight/2 + 240);
			break;
	}
}
...
function moveIndojet(){	
	if(!leftPressed && !rightPressed){
		if(planeTouchNav){
			if(touchCord.X < indoJet.posX) indoJetImage.draw("still", indoJet.posX, indoJet.posY, 2, 0, 0, 0);
			else indoJetImage.draw("still", indoJet.posX, indoJet.posY, 3, 0, 0, 0);
		} else indoJetImage.draw("still", indoJet.posX, indoJet.posY, 1, 0, 0, 0);
	}else{
		if(leftPressed && !rightPressed){
			indoJetImage.draw("still", indoJet.posX, indoJet.posY, 2, 0, 0, 0);
			if(indoJet.posX > 0) indoJet.posX -= indoJet.jetSpeed;
		}
		if(rightPressed && !leftPressed){
			indoJetImage.draw("still", indoJet.posX, indoJet.posY, 3, 0, 0, 0);
			if(indoJet.posX < gameWidth) indoJet.posX += indoJet.jetSpeed;
		}
		if(leftPressed && rightPressed) indoJetImage.draw("still", indoJet.posX, indoJet.posY, 1, 0, 0, 0);
	}
	if(upPressed && !downPressed) if(indoJet.posY > 0) indoJet.posY -= indoJet.jetSpeed;
	if(!upPressed && downPressed) if(indoJet.posY < gameHeight) indoJet.posY += indoJet.jetSpeed;
	followTouches();
}
...
document.getElementById("gameCanvas").addEventListener('touchstart', function(e){
	...
	switch (gameScreen){
		...
		case "selectaircraft" :
			if(checkCollision(touchCord.X, touchCord.Y, btnBack)) startScene("mainmenu");
			if(jetSelect >= 0 && jetSelect <= aircrafts.length-1){
				if(checkCollision(touchCord.X, touchCord.Y, btnPrev)){
					clickSound.play();
					if(jetSelect > 0) jetSelect -= 1;
					airCraftSelection();
				}
				if(checkCollision(touchCord.X, touchCord.Y, btnNext)){
					clickSound.play();
					if(jetSelect < aircrafts.length-1) jetSelect += 1;
					airCraftSelection();
				}
				if(checkCollision(touchCord.X, touchCord.Y, btnPurchase)){
					clickSound.play();
					if(jetSelect <= aircrafts.length-1 && score > aircrafts[jetSelect].price && !aircrafts[jetSelect].owned){
						aircrafts[jetSelect].owned = true;
						score -= aircrafts[jetSelect].price;
						localStorage.setItem(title+"Score", score);
						localStorage.setItem(title+"OwnedAircraft", JSON.stringify(aircrafts));
						airCraftSelection();
						return;
				...
				if(checkCollision(touchCord.X, touchCord.Y, btnSelect)){
					clickSound.play();
					if(jetSelect <= aircrafts.length-1){
						if(aircrafts[jetSelect].owned){
							currentJet = jetSelect;
							localStorage.setItem(title+"CurrentJet", currentJet);
							startScene("mainmenu");
						}
					}
				}
...
function followTouches(){
	if(planeTouchNav){
		if(touchCord.X < indoJet.posX - 20){
			leftPressed = true;
			rightPressed = false;
		...
		if(touchCord.Y < indoJet.posY - 20 + 100){
			upPressed = true;
			downPressed = false;
...
function fire(){
	if(indoJet.firingTimer >= indoJet.firingInterval){
		fireSound.play();
		var tempXPos = Math.random() * 3;
		switch(indoJet.bulletType){
			case 0 :
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX, "posY" : indoJet.posY, "moveX" : 0});
				break;
			case 1 :
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX - 25, "posY" : indoJet.posY, "moveX" : -1});
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX + 25, "posY" : indoJet.posY, "moveX" : 1});
				indoJet.bulletCounts -= 2;
				break;
			case 2 :
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX - 25, "posY" : indoJet.posY - 25, "moveX" : -1});
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX + 25, "posY" : indoJet.posY - 25, "moveX" : 1});
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX - 75, "posY" : indoJet.posY, "moveX" : -2});
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX + 75, "posY" : indoJet.posY, "moveX" : 2});
				indoJet.bulletCounts -= 4;
				break;
			case 3 :
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX, "posY" : indoJet.posY - 50, "moveX" : 0});
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX - 25, "posY" : indoJet.posY - 25, "moveX" : -2});
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX + 25, "posY" : indoJet.posY - 25, "moveX" : 2});
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX - 75, "posY" : indoJet.posY, "moveX" : -4});
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX + 75, "posY" : indoJet.posY, "moveX" : 4});
				indoJet.bulletCounts -= 5;
				break;
			case 4 :
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX, "posY" : indoJet.posY, "moveX" : 0});
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX - 25, "posY" : indoJet.posY, "moveX" : -1});
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX + 25, "posY" : indoJet.posY, "moveX" : 1});
				indoJet.bulletCounts -= 3;
				break;
			default :
				indoJet.bullets.push({"posX" : tempXPos + indoJet.posX, "posY" : indoJet.posY, "moveX" : 0});
				break;
		}
		indoJet.firingTimer = 0;
		if(indoJet.bulletType != 0) if(indoJet.bulletCounts <= 0){
			indoJet.bulletType = 0;
			indoJet.firingInterval = 10;
		}
	}
	indoJet.firingTimer++;
	if(indoJet.bullets.length > 0){
		for(var i = 0; i < indoJet.bullets.length; i++){
			var currentItem = indoJet.bullets[i];
			if(currentItem.posY < -100){
				indoJet.bullets.splice(i, 1);
				i -= 1;
			}else{
				currentItem.posY -= 14 + indoJet.firingSpeed;
				...
				if(checkBulletHit(indoJet.bullets, i)){
					indoJet.bullets.splice(i, 1);
					i -= 1;
				}
			}			
	...
	if(collectibles.items.length > 0){
		for(var i = 0; i < collectibles.items.length; i++){
			...
			if(currentItem.posY > gameHeight + 100){
				...
				if(checkCollision(currentItem.posX, currentItem.posY, indoJetImage)){
					collectSound.play();
					switch (currentItem.type){
						case 1 :
							indoJet.health = indoJet.maxHealth;
							break;
						case 2 : 
							indoJet.firingInterval = 6;
							indoJet.bulletType = 1;
							indoJet.bulletCounts = 100;
							break;
						case 3 : 
							indoJet.firingInterval = 7;
							indoJet.bulletType = 2;
							indoJet.bulletCounts = 200;
							break;
						case 4 : 
							indoJet.firingInterval = 5;
							indoJet.bulletType = 3;
							indoJet.bulletCounts = 150;
							break;
						case 5 : 
							indoJet.firingInterval = 3;
							indoJet.bulletType = 4;
							indoJet.bulletCounts = 350;
							break;
					}
					collectibles.items.splice(i, 1);
					i -= 1;
	...
	if(enemies.items.length > 0){
		for(var i = 0; i < enemies.items.length; i++){
			...
			if(currentItem.posY > gameHeight + 100){
				...
				ctx.fillStyle = "black";
				if(checkCollisionWp(currentItem.posX, currentItem.posY, indoJetImage)){
...
function updatePlayerStat(){
	ctx.textAlign = "left";
	if(indoJet.bulletType != 0){
		ctx.fillText("Ammo: " + indoJet.bulletCounts, 10, gameHeight - 20);
	}else{
	...
	if(indoJet.health <= 0){
		...
		ctx.fillText("Health: " + indoJet.health*10 + "%", gameWidth - 10, gameHeight - 20);
		ctx.fillStyle = "#e5e5e5";
		ctx.fillRect(0, gameHeight-5, gameWidth, gameHeight);
		var healthPerc = indoJet.health / indoJet.maxHealth;
...
function airCraftSelection(){
	if(jetSelect >= 0 && jetSelect <= aircrafts.length-1){		
		...
		ifImage.draw("still", gameWidth/2, gameHeight/2 - 200, 1, 0, 0, 0);
		ctx.globalAlpha = 1;
		if(!aircrafts[jetSelect].owned)	btnPurchase.draw("still", gameWidth/2, gameHeight - 200, 1, 0, 0, 0);
		else{
		...
		ctx.font = "bold 18px Arial";
		if(!aircrafts[jetSelect].owned){
			ctx.fillText("Price: " + aircrafts[jetSelect].price, gameWidth/2, gameHeight/2 - 150);
			ctx.fillText("Your Money: " + score, gameWidth/2, gameHeight/2 - 120);
		}
		jetselection.draw("still", gameWidth/2, gameHeight/2, jetSelect+1, 0, 0, 0);
